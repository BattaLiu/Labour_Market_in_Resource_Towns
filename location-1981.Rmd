...
tmp <- as.character(subdiv.81$prov)
...
metal.mine.80$PROVINCE <- as.character(metal.mine.80$PROVINCE)
...
match.mine.8081 <- inner_join(metal.mine.80, subdiv.81, by = c("PROVINCE"="provname", "CITYUP" = "city")) 
unmatch.mine.8081 <- anti_join(metal.mine.80, subdiv.81, by = c("PROVINCE"="provname", "CITYUP" = "city")) 

dup <- match.mine.8081 %>% 
	dplyr::select("PROVINCE", "COMPANY", "MINESITE", "LOCATION") %>% 
	duplicated(.)

duplicated.test.match <- unique(match.mine.8081[dup, ])

match.mine.8081 <- anti_join(match.mine.8081, duplicated.test.match, by = c("PROVINCE", "COMPANY", "MINESITE", "LOCATION")) #52 mines are matched directly and uniquely in 1980

unmatch.mine.8081 <- duplicated.test.match %>% 
	dplyr::select(names(unmatch.mine.8081)) %>% 
	unique(.) %>% 
	rbind(., unmatch.mine.8081)

write_csv(match.mine.8081, "/Users/batta/Dropbox/phd4/monopsony/mining-r/mine-30th-matched1-2.csv")
write_csv(unmatch.mine.8081, "/Users/batta/Dropbox/phd4/monopsony/mining-r/mine-30th-unmatched1-2.csv")
```
...
mine.to.scrap <- unmatch.mine.8081 %>% 
	dplyr::select(PROVINCE, COMPANY, MINESITE, LOCATION)
test <- mine.to.scrap %>% 
	filter(LOCATION !="")
		...
		if (advsearch.num[i] >1) { 
			...
			for (j in 1:length(links)){
				...
				if (check.prov==FALSE | length(check.table) == 0 | html_text(check.table)==""){
					...
					match <- str_detect(advsearch.words, locc)
					match.num <- sum(match)
...
write_csv(advsearch.output.test, "/Users/batta/Dropbox/phd4/monopsony/mining-r/advsearch-30-test-2.csv")
```
# Match with boundary file using GIS from mindat.org
```{r}
mine.sdf <- read_csv("/Users/batta/Dropbox/phd4/monopsony/mining-r/advsearch-30-test-2.csv")
...
map.sdf.81 <- readOGR("/Users/batta/Dropbox/phd4/monopsony/map/81_86_CSD_Canada/CSD81", "CSD81")
proj4string(mine.sdf) <- crs(map.sdf.81)
summary(map.sdf.06)
summary(map.sdf.81)
mine.attribute <- over(mine.sdf, map.sdf.81)
...
matched2.CSD <- matched2.CSD %>% 
	dplyr::select(PROVINCE, COMPANY, MINESITE, LOCATION, CSD81, CSD_NAME)
match.mine.8081 <- match.mine.8081 %>% 
...
colnames(match.mine.8081) <- c("PROVINCE", "COMPANY", "MINESITE", "LOCATION", "CSDUID", "CSDNAME")

match.mine.8081.2 <- rbind(match.mine.8081, matched2.CSD) #74 out of 104 matched with CSD
duplicated(match.mine.8081.2 %>% 
					 	dplyr::select("PROVINCE", "COMPANY", "MINESITE", "LOCATION"))
unmatch.mine.8081.2 <- anti_join(metal.mine.80, match.mine.8081.2, by = c("PROVINCE", "COMPANY", "LOCATION")) #Why there are some unmatched minesite?
write_csv(match.mine.8081.2, "/Users/batta/Dropbox/phd4/monopsony/mining-r/mine-30th-matched2-2.csv")
write_csv(unmatch.mine.8081.2, "/Users/batta/Dropbox/phd4/monopsony/mining-r/mine-30th-unmatched2-2.csv")
```
* Get information of mines in mine-30th-unmatched2.csv manually and then find CSD code in 2006 boundary file
```{r}
matched3 <- read_csv("/Users/batta/Dropbox/phd4/monopsony/mining-r/mine-30th-unmatched.csv") # This is the full list of mine data in 1980 manually get.
unmatch.mine.8081.2 <- read_csv("/Users/batta/Dropbox/phd4/monopsony/mining-r/mine-30th-unmatched2-2.csv") # in order to get the same column class.
matched3.1 <- inner_join(unmatch.mine.8081.2, matched3, by = c("PROVINCE", "COMPANY", "MINESITE", "LOCATION"))
...
coord.test.dec$LATITUDE <- as.numeric(coord.test.dec$latitude)
coord.test.dec$LONGITUDE <- as.numeric(coord.test.dec$longitude)
...
proj4string(matched3.3) <- crs(map.sdf.81)
matched3.3.attribute <- over(matched3.3, map.sdf.81)
...
matched3.4$CSDUID <- as.numeric(as.character(matched3.4$CSD81))
CSDbind <- matrix(nrow = nrow(matched3.4))
for (i in 1:nrow(matched3.4)) {
	CSDbind[i] <- matched3.4$`csd81-book`[i]
	if (is.na(CSDbind[i])) {
		CSDbind[i] <- matched3.4$CSDUID[i]
		if (is.na(CSDbind[i])) {
			CSDbind[i] <- matched3.4$`csd12-book`[i]
			if (is.na(CSDbind[i])) {
				CSDbind[i] <- matched3.4$csd12[i]
			}
		} 
	}
}
matched3.4$CSDBIND <- CSDbind
bababa <- matched3.4 %>% 
	filter(!is.na(CSDBIND))
```

```{r}
matched3.CSD <- matched3.4 %>% 
	dplyr::select(PROVINCE, COMPANY, MINESITE, LOCATION, CSDBIND)
matched3.CSD$CSDBIND <- as.character(matched3.CSD$CSDBIND)
csdname <- left_join(matched3.CSD, subdiv.81, by = c("CSDBIND" = "sgcfull"))
matched3.CSD$CSDNAME <- csdname$csdname
colnames(matched3.CSD) <- c("PROVINCE", "COMPANY", "MINESITE", "LOCATION", "CSDUID", "CSDNAME")

match.mine.8081.3 <- rbind(match.mine.8081.2, matched3.CSD) #all 104 mines matched with CSD
duplicated(match.mine.8081.2, by = c("PROVINCE", "COMPANY", "MINESITE", "LOCATION"))
unmatch.mine.8081.3 <- anti_join(metal.mine.80, match.mine.8081.3, by = c("PROVINCE", "COMPANY","LOCATION"))
write_csv(match.mine.8081.3, "/Users/batta/Dropbox/phd4/monopsony/mining-r/mine-30th-matched3-2.csv")
write_csv(unmatch.mine.8081.3, "/Users/batta/Dropbox/phd4/monopsony/mining-r/mine-30th-unmatched4-2.csv")
```
