...
* used variables: naicsh, pcsd, wages, age, hgradr, ps_otr, ps_uvr, hlos, hcdd
clear
log using "P:\Liu_5114\log\2006-stata-data-cleaning-weighted.log", replace
use "P:\Liu_5114\data\cen_2006_f1_v2.dta"
rename pwsgc pow
/* check the percentage of people working in industry 11 (metal ore mining) */
gen indmetal = 1 if naicsh ==11
replace indmetal = 0 if naicsh!=11
...
gen indavail = 1 if naicsh != 291
replace indavail = 0 if naicsh == 291
...
gen pubsec = 1 if naicsecf == 16 | naicsecf == 17 | naicsecf == 21
gen pvtsec = 1 if pubsec !=1 & naicsecf != 1 
gen pubwkwages = wkwages if pubsec == 1
gen pvtwkwages = wkwages if pvtsec == 1

gen prisec = 1 if naicsecf >1 & naicsecf <4
gen secsec = 1 if naicsecf > 3 & naicsecf <7
gen tersec = 1 if naicsecf > 6 & naicsecf <22
...
gsort pcsd
gen year = 2006
save "P:\Liu_5114\result\labor market outcome\2006-labor-outcome-weighted-compw2-test.dta", replace
log close
